...
import type { ITaskReq } from '@/types/task'
...
async function onSubmit() {
  ...
  if (state.createTaskStatus === STATUS.COMPLETE) {
  ...
  <Dialog v-model:open="isOpenModal">
    <div class="w-full flex justify-center m-5">
      <DialogTrigger as-child @click="() => (isOpenModal = true)">
        <Button variant="outline" class="w-1/5"> Добавить Задачу </Button>
      </DialogTrigger>
    </div>
    <DialogContent class="sm:max-w-[425px]">
      ...
      <div class="grid gap-4 py-4">
        ...
        <Alert variant="destructive" v-if="state.createTaskStatus === STATUS.ERROR">
      ...
      <DialogFooter>
        <Button
          :disabled="state.createTaskStatus === STATUS.LOADING"
          type="submit"
          @click="onSubmit"
        >
          <Loader2
            class="w-4 h-4 mr-2 animate-spin"
            v-if="state.createTaskStatus === STATUS.LOADING"
          />
          Создать
        </Button>
      </DialogFooter>
